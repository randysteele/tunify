...
function getTracks() {
  fetch(tracks_URL)
    ...
    function postFetch(name, playlist_id, track, artist, title){
    fetch(BACKEND_URL, {
      method: "POST",
   ...
   function postTrack(title, artist){
      ...
      .then(response => response.json())
      .then(trk => {          
        const trackData = trk.data;      
        let newTrack = new Track(trackData, trackData.attributes);
       // let newTrack = new Track(trackData, trackData.attributes)
        document.getElementById("track-container").innerHTML += newTrack.renderPlaylistCard() ;
