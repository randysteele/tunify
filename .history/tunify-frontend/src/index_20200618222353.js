...
function getPlaylists() {
  fetch(BACKEND_URL)
    ...
    function selectTracks(){
      ...
      .then(response => response.json())
      .then(track  => {
          track.data.forEach(playlist => track => {                        
           let newPlaylist = new Playlist(playlist, playlist.attributes)  
            console.log('all the tracks')
          document.getElementById("playlist-container").innerHTML += newPlaylist.renderPlaylistCard(selectTracks);
          }
      )}
  )
    }
