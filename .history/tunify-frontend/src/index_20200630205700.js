  ...
  fetch(BACKEND_URL)
      ...
      .then(list  => {
        list.data.forEach(playlist =>  {      
           ...
           let trackDetails = [playlist.attributes.tracks, playlist.attributes.name].toString;  
           let i;
           for (i = 0; i < playlist.length; i++) {
           for (let t = 0; playlist[i].length; t++ )
           trackDetails.push(playlist.attributes.tracks[i][t].title, playlist.attributes.tracks[i][t].artist)
          }
