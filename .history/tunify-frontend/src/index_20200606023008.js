...
function getPlaylists(){
  ...
  list.data.forEach(playlist=> {
    const playlistMarkup = `
       <div data-id=${tracks.id}>
        <h4>${playlist.attributes.name}</h4>
        <h4>${playlist.attributes.artist}</h4>     
        <a href=${playlist.attributes.image}>      
        <a href=${playlist.attributes.preview}>  
        <select id=${playlist.attributes.name}</select>
        <button data-id=${tracks.id}>add</button>
