  ...
  fetch(BACKEND_URL)
      ...
      .then(list  => {
        list.data.forEach(playlist =>  {      
           let newPlaylist = new Playlist(playlist, playlist.attributes, playlist.attributes.name);  
           let trackDetails = [playlist.attributes.tracks];
   
           let i;
           for (i = 0; i < playlist.length; i++) {
            
             playlist.attributes.tracks[i].value, playlist.attributes.tracks[i].title.value
          }          
