...
function getTracks() {
  fetch(tracks_URL)
    ...
    function createFormHandler(e) {
      ...
      postFetch(nameInput, titleValue, artistValue, playlistId)   
      postTrack(artistValue, titleValue, nameInput, playlistId) 
    }

    function postFetch(name, title, artist){
    fetch(BACKEND_URL, {
      method: "POST",
  ...
  function postTrack(title, artist, playlist_id, name){
    fetch(tracks_URL, {
      method: "POST",
      headers: {"Content-Type": "application/json"},
      body: JSON.stringify({
