      ...
      .then(list  => {
       list.data.map((playlist, index)  =>  {      
          let newPlaylist = new Playlist(playlist, playlist.attributes);
            ...
            let withTracks = []
            withTracks.push(playlist.attributes.tracks)
          //  withTracks.push(playlist.attributes.tracks[index].artist)
              //  playlist.attributes.tracks[index].title )

         // debugger
          
       document.getElementById('tracks-container').innerHTML += withTracks[index] += newPlaylist.renderPlaylistCard();  
