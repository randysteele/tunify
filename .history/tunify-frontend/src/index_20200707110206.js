...
document.addEventListener('DOMContentLoaded', () => {
 ...
 function getPlaylists() {
    fetch(BACKEND_URL)
        ...
        .then(list  => {
          let i;
          for(i = 0; i < list.data.tracks.length; i++)       
          list.data.attributes.tracks[i].map(playlist =>  { 
            debugger
            

           
  
          let newPlaylist = new Playlist(playlist, playlist.attributes, track, track.attributes);  
