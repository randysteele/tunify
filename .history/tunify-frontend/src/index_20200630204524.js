  ...
  fetch(BACKEND_URL)
      ...
      .then(list  => {
        list.data.forEach(playlist =>  {      
           ...
           let trackDetails =  Playlist[playlist.attributes.tracks, playlist.attributes.name];  
           let i;
           for (i = 0; i < playlist.length; i++) {
           for (let t = 0; playlist[i].length; t++ )
          return trackDetails.push(playlist.attributes.tracks[i][t++].title)
          }
          
      document.write(trackDetails)
