...
function getPlaylists() {
  fetch(BACKEND_URL)
      ...
      .then(list  => {
          ...
          let withTracks = []
          withTracks.push(playlist.attributes.tracks.artist, playlist.attributes.tracks.title, playlist.attributes.name)

         document.getElementById('tracks-container').innerHTML += withTracks += newPlaylist.renderPlaylistCard();   
  ...
  fetch(tracks_URL)
      ...
      .then(trk  => {
          ...
          let newTrack = new Track(track, track.attributes);
           let withTracks = [tracks.attributes]
