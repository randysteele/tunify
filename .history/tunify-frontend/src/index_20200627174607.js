...
function getTracks() {
  fetch(tracks_URL)
      ...
      .then(trk  => {
          ...
          let newTrack = new Track(track, track.attributes, track.playlist_id);
          let withTracks = []
          withTracks.push(track.attributes.artist, track.attributes.title)
          
          document.getElementById('tracks-container').innerHTML += newTrack.renderTrackCard(track.playlist_id);   
