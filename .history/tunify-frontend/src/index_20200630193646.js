...
function getPlaylists() {
  fetch(BACKEND_URL)
      ...
      for (i = 0; i < playlist.attributes.tracks; i++) {
         console.log(playlist.attributes.tracks[i].title)
          }
          
         document.getElementById('playlist-container').innerHTML += console.log(playlist.attributes.tracks[i].title) += newPlaylist.renderPlaylistCard();         
        })       
        }
  )
}
