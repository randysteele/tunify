...
function getTracks() {
  fetch(tracks_URL)
    ...
    function createFormHandler(e) {
     ...
     const playlistId = document.getElementById('title-input').value
     const titleInput = document.getElementById('title-input').value
     const artistInput = document.getElementById("artist-input").value

      postFetch(nameInput, artistInput, titleInput)   
      postTrack(playlistId, artistInput, titleInput) 
    }
  ...
  function postTrack(title, artist){
    ...
    .then(track => {          
     // let trackData = track.data;      
      let newTrack = new Track(track.data);
       // debugger
      document.getElementById('tracks-container').innerHTML += newTrack.renderTrackCard() ;
