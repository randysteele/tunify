...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const createPlaylistForm  = document.getElementById("playlist-container").innerHTML += playlistMarkup;
  createPlaylistForm.addEventListener("submit", (e) => createFormHandler(e)        
  );

  const addSong = document.getElementById("add-button.playlist.id");
  addSong.addEventListener("submit", (e) => playlist-container(e))
...
function getPlaylists(){
  ...
  list.data.forEach(playlist=> {
    const playlistMarkup =     `    
       ...
       <input id="add-button.playlist.id" type ="submit" name="submit" value="Add ${playlist.attributes.name} To ${playlist.name}"> 
