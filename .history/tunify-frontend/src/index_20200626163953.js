      ...
      .then(list  => {
          list.data.forEach(playlist =>   { 
            ...
            for (i = 0; i < playlist.attributes.tracks; i++) {
                ...
                playlist.attributes.tracks[i]
             // until i > playlist.attributes.tracks 
               }                         
          let newPlaylist = new Playlist(playlist, playlist.attributes);

           let withTracks = []
           withTracks.push(playlist.attributes.tracks[i].title, playlist.attributes.tracks[i].artist, playlist.attributes.tracks[i].title, playlist.attributes.tracks[i].artist)
         ...
         // playlist.push(playlist)
          ...
          document.getElementById("playlist-container").innerHTML += withTracks += newPlaylist.renderPlaylistCard(withTracks);   
